...
def part_1_b_i(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            if len(row) >= 3:
                connection.execute("INSERT INTO incidents VALUES (?,?,?)", (row[0], row[1], row[2]))
    connection.commit()
    ######################################################################
...
def part_1_b_ii(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            if len(row) >= 4:
                connection.execute("INSERT INTO details VALUES (?,?,?,?)", (row[0], row[1], row[2], row[3]))
    connection.commit()
    ######################################################################
...
def part_1_b_iii(connection: Connection, path: str) -> None:
    ...
    with open(path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            if len(row) >= 7:
                connection.execute("INSERT INTO outcomes VALUES (?,?,?,?,?,?,?)", (row[0], row[1], row[2], row[3], row[4], row[5], row[6]))
    connection.commit()
    ######################################################################
...
def part_3() -> str:
    ############### EDIT SQL STATEMENT ###################################
    query = "SELECT ROUND( 100.0 * ( SELECT COUNT(*) FROM incidents WHERE date >= '2018-01-01' AND date <= '2020-12-31' ) / (SELECT COUNT(*) FROM incidents) , 2);"
    ######################################################################
    return query
...
def part_4() -> str:
    ############### EDIT SQL STATEMENT ###################################
    query = "SELECT transport_mode, COUNT(*) AS count FROM details WHERE detection = 'Intelligence' AND transport_mode IS NOT NULL AND transport_mode <> '' GROUP BY transport_mode ORDER BY count DESC LIMIT 3;"
    ######################################################################
    return query
...
def part_5() -> str:
    ############### EDIT SQL STATEMENT ###################################
    query = "SELECT detection, COUNT(*) AS count, ROUND(AVG(num_ppl_arrested), 2) AS avg_ppl_arrested FROM details INNER JOIN outcomes USING (report_id) WHERE num_ppl_arrested > 0 GROUP BY detection HAVING COUNT(*) >= 100 ORDER BY avg_ppl_arrested DESC LIMIT 3;"
    ######################################################################
    return query
...
def part_7_a() -> str:
    ############### EDIT SQL STATEMENT ###################################
    query = "CREATE VIEW fines AS SELECT incidents.report_id, incidents.date, outcomes.num_ppl_fined, outcomes.fine FROM incidents INNER JOIN outcomes USING (report_id) WHERE num_ppl_fined > 0;"
    ######################################################################
    return query
...
def part_8_b() -> str:
    ############### EDIT SQL STATEMENT ############################
    query = "INSERT INTO incident_overviews (report_id, subject) SELECT report_id, subject FROM details;"
    ######################################################################
    return query
