...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        # Return all nodes with the maximum degree
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list[int]=None) -> list:
        ...
        """
        # Initialize exclude_ids if None
        if exclude_ids is None:
            exclude_ids = []
        
        # Make API request to get movie credits
        ...
        # If limit is specified, filter to only include cast members with order < limit
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        """
        # Make API request to get person's movie credits
        conn = http.client.HTTPSConnection("api.themoviedb.org")
        ...
        for credit in cast:
            ...
            # Skip if release_date is empty
            if not release_date:
                continue
            
            # Check start_date condition
            ...
            # Add to filtered list with required fields
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='3dccc98ee58c21e266bab0461fec094f')

    # ===== TESTS =====
    print("Running tests...")
    
    # Test 1: Test Graph class methods
    test_graph = Graph()
    test_graph.add_node(id='1', name='Actor One')
    test_graph.add_node(id='2', name='Actor Two')
    test_graph.add_edge(source='1', target='2')
    
    print(f"Test 1 - Graph basics:")
    print(f"  Total nodes: {test_graph.total_nodes()} (expected: 2)")
    print(f"  Total edges: {test_graph.total_edges()} (expected: 1)")
    
    # Test 2: Test duplicate prevention
    test_graph.add_node(id='1', name='Actor One')  # Should not be added again
    print(f"Test 2 - Duplicate prevention:")
    print(f"  Total nodes after adding duplicate: {test_graph.total_nodes()} (expected: 2)")
    
    # Test 3: Test max_degree_nodes
    test_graph.add_node(id='3', name='Actor Three')
    test_graph.add_edge(source='1', target='3')
    print(f"Test 3 - Max degree nodes: {test_graph.max_degree_nodes()} (actor 1 should have degree 2)")
    ...
    # BUILD BASE GRAPH
    # Get all movie credits for Laurence Fishburne from 1999
    movies_1999 = tmdb_api_utils.get_movie_credits_for_person('2975', start_date='1999-01-01', end_date='1999-12-31')
    
    # Track nodes added in the base graph
    new_nodes_base = set()
    
    for movie in movies_1999:
        try:
            movie_id = str(movie['id'])
            # Get cast members with order 0-4 (first 5 actors)
            cast = tmdb_api_utils.get_movie_cast(movie_id, limit=5)
            ...
            for cast_member in cast:
                ...
                # Add edge between Laurence Fishburne and this actor
                graph.add_edge(source='2975', target=actor_id)
        except Exception as e:
            # Skip movies that don't return cast data
            continue
    
    # EXPANSION LOOP - DO 2 TIMES
    nodes_to_process = new_nodes_base
    
    for iteration in range(2):
        new_nodes_iteration = set()
        
        for node_id in nodes_to_process:
            try:
                # Get movie credits for this actor from 1999
                movies_for_actor = tmdb_api_utils.get_movie_credits_for_person(node_id, start_date='1999-01-01', end_date='1999-12-31')
                
                for movie in movies_for_actor:
                    try:
                        movie_id = str(movie['id'])
                        ...
                        for cast_member in cast:
                            ...
                            if (co_actor_id, co_actor_name) not in graph.nodes:
                                ...
                                new_nodes_iteration.add(co_actor_id)
                            
                            # Add edge if it doesn't already exist
                            graph.add_edge(source=node_id, target=co_actor_id)
                    except Exception as e:
                        # Skip movies that don't return cast data
                        continue
            except Exception as e:
                # Skip actors that have issues retrieving credits
                continue
        
        # Update nodes_to_process for next iteration
        nodes_to_process = new_nodes_iteration
